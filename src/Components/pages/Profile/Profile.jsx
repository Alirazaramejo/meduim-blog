    ...
    return (
        <section className='size flex gap-[4rem] relative '>
            ...
            <div className="mt-[9rem] flex-[2]">
                ...
                <div className="flex items-center gap-5 mt-[1rem] border-b border-gray-200 mb-[3rem]">
                    {activity.map((item, index) => (
                        <div key={index} className={`py-[0.5rem] ${item.title === currentActive.title ? 'border-b border-gray-500' : ''}`}>
            ...
            <Modal modal={modal} setModal={setModal}>
                <div className={`flex-[1] border-l border-gray-300 p-[2rem] z-10 fixed right-0 bottom-0 top-0 w-[10rem] bg-white md:relative ${modal ? 'translate-x-0' : 'translate-x-[100%] md:translate-x-0'} transition-all duration-500`}>
                    {/* Icons make to close the profile */}
                    <div className="pb-4 text-right ">
                        <button onClick={() => setModal(false)} className='inline-block md:hidden '><LiaTimesSolid /></button>
                    </div>
                    {/* Profile details make */}
                    <div className="sticky top-7 flex-col justify-between">
                        <img className='w-[3.5rem] h-[3.5rem] object-cover rounded-full' src={profileImg} alt="profile-Img" />
                        <h2 className='py-2 font-bold capitalize'>Ali Raza</h2>
                        <p className='text-gray-500 first-letter:uppercase text-sm'>I am Ali Raza</p>
                    ...
                    {/* Nav data from datajs file get */}
                     <div className="flex-[1] flex items-center flex-wrap gap-3 pt-8">
                        {discoverActions.map((item, index) => (
                            <button key={index} className='text-xs text-sm text-black'>{item}</button>
                        ))}
                    </div>
                    </div>
                </div>
